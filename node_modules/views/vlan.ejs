...
<body>
    ...
    <form action="/" method="post">
        ...
        <input type="submit" value="Generate" onclick="displayBtnGenText()">
    </form>
    <p>
        <%= error %>
    </p>
                ...
                <%=vlanConfig[i].vlanNumber %>: <input value="<%=vlanConfig[i].name %>" name="vlanName" type="text">
                    ...
                    <% for (var index=0 ; vlanConfig[i].interfaceConfig.length> index; index++){%>
                        ...
                        <input type="text" name="interfaceName"
                            value="<%= vlanConfig[i].interfaceConfig[index].interfaceName %>" placeholder="">
                        ...
                        <button type="submit" value="<%=vlanConfig[i].vlanNumber %>"
                            name="deleteInterface">Eliminar</button>
                        <% } %>
    ...
    <form action="/generateText" method="post">
        <button name="generateText" id="generateT" value="true" type="submit">Generate text</button>
    </form>
    ...
    <div id="output">
        ...
        <form action="" method="POST">
            ...
            <p>
                <button onclick="generateText()">Create Vlans</button>
        </form>
    </div>
</body>
...
<script>
    ...
    function displayBtnGenText() {
        document.getElementsById("generateT").style.color = "blue";
        alert("entrei")
    }
    ...
    function insertVlan() {
        ...
        //let stringed = vlans.JSON.stringify(vlans)

        //localStorage.setItem('generated', stringed);
